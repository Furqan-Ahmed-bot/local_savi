...
class UserServices {
  ...
  signupService({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // pass.value = password;
        Get.close(1);
        Get.snackbar("Suucess", "OTP sent to email",
            backgroundColor: ColorUtils.white);
  ...
  getCancelJobReasons({String reason = "CANCEL"}) async {
    try {
      ...
      if (response.statusCode == 200) {
        String cancelReasonsResponseBody =
            await response.stream.bytesToString();

        Map<String, dynamic> jsonResponse =
            json.decode(cancelReasonsResponseBody);
  ...
  Future applyPerformerJob({required context, required jobId}) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
                    ...
                    child: Row(
                      ...
                      children: [
                        GestureDetector(
                          onTap: () {
                            Get.close(1);
                            PHomeController pHomeController =
                                Get.find<PHomeController>();
        ...
        resposeDAta['message'];
        Get.close(1);
        Get.snackbar("Alert", "${resposeDAta['message']}",
            backgroundColor: ColorUtils.white);
  ...
  createDispute({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        Get.close(1);
        Get.snackbar("Success", "Dispute Successful",
            backgroundColor: ColorUtils.white);
        showDialog(
            context: context,
            builder: (BuildContext context) {
  ...
  editUerJobService({
    ...
    try {
      ...
      var request = http.MultipartRequest(
          'PATCH', Uri.parse(UserUrls.updateUserJob + jobId));
      request.fields.addAll({
        'worker_type': workerType,
        'title': title,
        'description': desc,
      ...
      if (res.statusCode == 200) {
        ...
        Get.snackbar("Success", "Job update successfully",
            backgroundColor: ColorUtils.white);
      } else {
        jsonResponse['message'];
